 ...
 public:
  ...
  void generate_connections(){ //generates geometry topology
    ...
    for (int c=0; c<CN.n; c++){ //layer connections
        ...
        int fxtxconn[TX];  // to count the connection number each neuron received
	double maxconn=TX;
	int flag=0;
	for(int i=0;i<TX;i++)    //--- then fill it with values
	{
  		fxtxconn[i] = 0; /*corresponding data point*/;
        }
...
*/
	...
	//	cerr<<radius_x<<endl;
       ...
       if ((CT.cell[CN.C[c].from].name!=CT.cell[CN.C[c].to].name)&&(CT.cell[CN.C[c].to].name=="CX"||CT.cell[CN.C[c].to].name=="CXa"||CT.cell[CN.C[c].to].name=="CX6")&&(TX!=FX))
       {
	 //shift the current number of location in input layer 
	  sc_x=sc_x+TX/FX*0.5;   

	 //deal with the unequal number connection issues
	 // Each neuron should receive the maximal connection numbers within 2*radius/scale, where scale= TX/FX
         maxconn=2*radius_x/(TX/FX);
	 flag=1;
       }
      
       // special case: from CXa to CX with radius =0, each one PY in CX connect to each one PY in CXa
       if(CT.cell[CN.C[c].from].name=="CXa"&&radius_x==0)
       {
	  radius_x1=0;
	  radius_x2=0;
       }
	 ...
	 //	 boundarybias=-2; //for flow(mirror) boundary connection
      ...
      for (int ii=radius_x1; ii<=radius_x2; ii++) //particular connections between neurons
	     ...
	     //  if (fxtxconn[tx]>maxconn-1&&flag==1) continue; // deal with the unequal number connection issues
