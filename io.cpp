...
void load_input_params(
  ...
  add_int_param(  stimType   );
  add_double_param(  dtime   );
...
void print_occ(Pair *cell_sizes){
  ...
  for(i = 0; i < cell_sizes[E_CX].x; ++i){
    for(j = 0; j < cell_sizes[E_CX].y; ++j){
     ...
     print_receive_gsynapse_index(i, j, E_CX, E_CX, E_AMPA_D3, f20);
    }    
  }
  fprintf(f20,"\n");
  ...
  for(i = 0; i < cell_sizes[E_CXa].x; ++i){
    for(j = 0; j < cell_sizes[E_CX].y; ++j){
      print_receive_gsynapse_index(i, j, E_CXa, E_CX, E_AMPA_D3, f21);
    }    
  }
  fprintf(f21,"\n");
  ...
  for(i = 0; i < cell_sizes[E_CX].x; ++i){
    for(j = 0; j < cell_sizes[E_CXa].y; ++j){
      print_receive_gsynapse_index(i, j, E_CX, E_CXa, E_AMPA_D3, f22);
    }    
  }
  fprintf(f22,"\n");
  ...
  for(i = 0; i < cell_sizes[E_CXa].x; ++i){
    for(j = 0; j < cell_sizes[E_CXa].y; ++j){
      //     print_receive_gsynapse(i, j, E_CXa, E_AMPA_D3, f23);
      print_receive_gsynapse_index(i, j, E_CXa, E_CXa, E_AMPA_D3, f23);
    }    
  }
  fprintf(f23,"\n");
