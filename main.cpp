...
double input_strength,input_start_neuron,input_end_neuron,dtime;
int stimType;
...
int one_state=0;
int awake_end=210000;//210000;
int stage2_end=210000;//210000;//510000;
int stage3_end=510000;//510000;
int stim_start=awake_end;
int stim_end=510000;
/*
//awake to stage 2
int awake_end;    //15000
...
void print_receive_gsynapse_index(int m , int n, enum Cell_Type from_type, enum Cell_Type type, enum Syn_Type stype, FILE *fp){
  ...
  for(int i =0; i < cells[group_index]->num_syns; i++){
    if ((cells[group_index]->syns[i]->type == stype) && ((cells[group_index]->syns[i]->from_type == from_type))){   
      ...
      fprintf(fp,"%d %lf ", m, ((AMPA_D3*)(cells[group_index]->syns[i]))->g_AMPA0);
    }
  }
  //message = cells[group_index]->base_cell->get_v_soma();
}
...
int main(int argc,char **argv){
  ...
  load_input_params(argc,argv,tmax,t3D,ttime,num_mp_threads,print_c_sten,fre_print_cs,LFP.local_field_effect,LFP.lfp_scale,LFP.num_field_layers,homeo.boost,homeo.amp_boost,homeo.con_boost,homeo.fre_boost,homeo.target_f,homeo.fre_window,homeo.num_regions,input_strength,input_start_neuron,input_end_neuron,stimType,dtime);
  ...
  double gkl_s3            = gkl_awake_fix*s3_scale*0.95; //0.35; //correct1_func(s2_ach_fac    ,0.0);
 ...
 // double awake_AMPAd2      =0.2; //wake_ach_fac;
  ...
  double s2_AMPAd2         =awake_AMPAd2_fix*s2_scale*0.85; //0.28; //s2_ach_fac;
  double s3_AMPAd2         =awake_AMPAd2_fix*s3_scale*1.14; //0.28; //s2_ach_fac;
